...
# Find the today card section (between 今日 · 四月廿三 and next <div class="card">)
pattern = r'(<div class="card-title"><span class="icon">🧧</span>今日 · 四月廿三</div>)(.*?)(<div class="card">)'
m = re.search(pattern, content, re.DOTALL)

if not m:
    print('ERROR: today card not found', file=sys.stderr)
    sys.exit(1)

# Build clean today card body with proper text labels
# Data: based on siyueershisan detailData added in update_jun8.py
new_today_body = '''
...
# Replace
new_section = m.group(1) + new_today_body + m.group(3)
